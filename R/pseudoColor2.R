pseudoColor2 <- function(x, min=0, max=255) {
  x <- x*255+1
  ...
  if(max==255){
    ...
    lut.g <- c(seq(0, min, by=1), up, 
               rep(255, (max-min-length(up)*2)), down)
    ...
    x.b <- matrix(lut.b[x], nrow(x), ncol(x))
      ...
      lut.r <- c(seq(0, min, by=1), rep(0, half), up, 
                 rep(255, (max-min-half-length(up)-1)),
                 seq(max, 255, by=1))
      x.r <- matrix(lut.r[x], nrow(x), ncol(x))
      lut.g <- c(seq(0, min, by=1), up, 
                 rep(255, (max-min-length(up)*2-1)), 
                 down, seq(max, 255, by=1))
      x.g <- matrix(lut.g[x], nrow(x), ncol(x))
      lut.b <- c(seq(0, min, by=1), rep(255, quat), down, 
                 rep(0, (max-min-length(down)-1-quat)), 
                 seq(max, 255, by=1))
      x.b <- matrix(lut.b[x], nrow(x), ncol(x))
      }
