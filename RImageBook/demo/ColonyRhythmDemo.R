...
display(normalize(imagea), "Nomalized image")
mask1 <- thresh(imagea, 10, 10, 0.00003)
display(mask1, "Binary image")
mask2 <- matrix(0, 512, 512)
mask2 <- drawCircle(mask2, 250, 260, 190, col=1, fill=TRUE)
display(mask2, "Circular mask")
mask3 <- mask1 * mask2
display(mask3, "Masked image")
mask3 <- bwlabel(mask3)
...
mask3 <- mask3 > 0
mask3 <- bwlabel(mask3)
display(mask3, "Isolated colonies")
...
xy <- hf[,c('g.x','g.y')]
xy <- xy +5
display(drawtext(mask3, xy, labels=labels, col="white"), "Labeled colonies")
...
display(mask4, "Peudo-color coding of period")
detach(package:waveclock)
