tracking2 <- function (mask, maxdist = 20, bin = 3, interval = 0.1, unit = 1, size = NULL) {
  ...
  if (is.null(size) == T) {
    for (i in 1:(nf - 1)) {
      distance <- distmat(ftrs[[i]][, "m.cx"], ftrs[[i]][,"m.cy"], 
                          ftrs[[i + 1]][, "m.cx"], ftrs[[i + 1]][, "m.cy"])
  ...
  dispx <- sapply(objid, function(x) c(diff(xy[which(xy[, 1] == x), ][, 2], lag = bin), 
                                       rep(NA, bin)))
  dispy <- sapply(objid, function(x) c(diff(xy[which(xy[, 1] == x), ][, 3], lag = bin), 
                                       rep(NA, bin)))
  vx <- dispx/interval
  vy <- dispy/interval
  ...
  ax <- sapply(objid, function(x) c(diff(temp[which(temp[,1] == x), ][, 8], lag = bin), 
                                    rep(NA, bin)))
  ay <- sapply(objid, function(x) c(diff(temp[which(temp[,1] == x), ][, 9], lag = bin), 
                                    rep(NA, bin)))
  as <- sapply(objid, function(x) c(angularVelocity(xy[which(xy[, 1] == x), c(2, 3)], 
                                                    bin = bin, int = interval), rep(NA, 2 * bin)))
  res <- cbind(temp, as.vector(ax/interval), as.vector(ay/interval), as.vector(as))
  res <- data.frame(obj = res[, 1], x = res[, 2], y = res[,3], distance = res[, 4], 
                    d.x = res[, 5], d.y = res[, 6],  speed = res[, 7], 
                    v.x = res[, 8], v.y = res[, 9], acc.x = res[, 10], acc.y = res[, 11], 
                    v.ang = res[, 12])
